"""
BingX WebSocket market-data provider.
Maintains a real-time kline cache via BingX WebSocket, reducing REST API calls.
...
BINGX_MARKET_API_URL = os.getenv("BINGX_MARKET_API_URL", "https://open-api.bingx.com")


class BingXWebSocketDataProvider:
    """
    ...
    def _backfill_symbol(self, symbol: str):
        ...
        market_url = f"{BINGX_MARKET_API_URL}/openApi/swap/v3/quote/klines"
        params = {
...
WebSocketDataProvider = BingXWebSocketDataProvider


_provider: Optional[BingXWebSocketDataProvider] = None
...
def get_ws_provider() -> Optional[BingXWebSocketDataProvider]:
...
def start_ws_provider(symbols: List[str], interval: str = "5m"):
    ...
    _provider = BingXWebSocketDataProvider(manager=None)
